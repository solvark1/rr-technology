...
/**
 * \file        htdocs/custom/verleih/class/verleihitemtype.class.php
 * \ingroup     verleih
 * \brief       CRUD class for VerleihItemType (Objekttyp/Gerätekatalog)
 */

require_once DOL_DOCUMENT_ROOT.'/core/class/commonobject.class.php';

/**
 * Class for VerleihItemType
 */
class VerleihItemType extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'verleihitemtype';

	/**
	 * @var string Name of table without prefix where object is stored.
	 */
	public $table_element = 'verleih_itemtype';

	/**
	 * @var string String with name of icon for this object.
	 */
	public $picto = 'fa-tablet-alt';
	...
	public $fields = array(
		...
		'ref' => array('type' => 'varchar(32)', 'label' => 'Ref', 'enabled' => 1, 'position' => 10, 'notnull' => 1, 'visible' => 1, 'index' => 1, 'searchall' => 1, 'showoncombobox' => 1, 'css' => 'minwidth100'),
		...
		'itemcategory' => array('type' => 'varchar(64)', 'label' => 'VerleihItemCategory', 'enabled' => 1, 'position' => 30, 'notnull' => 0, 'visible' => 1, 'searchall' => 1, 'css' => 'minwidth150'),
		'manufacturer' => array('type' => 'varchar(128)', 'label' => 'VerleihManufacturer', 'enabled' => 1, 'position' => 40, 'notnull' => 0, 'visible' => 1, 'searchall' => 1, 'css' => 'minwidth150'),
	...
	{
		$url = dol_buildpath('/verleih/itemtype_card.php', 1).'?id='.$this->id;
		$result = '<a href="'.$url.'">';
		if ($withpicto) {
	 ...
	 *        checkbox state), 'itemcount' (int, avoids a per-card query when supplied)
	 * @return string HTML
	 */
	public function getKanbanView($option = '', $arraydata = null)
	{
		...
		$return .= '</div>';
		$return .= '<div class="vl-sub">'.dol_escape_htmltag($this->label).'</div>';
		$return .= '<div class="vl-body">';
		if (!empty($this->itemcategory)) {
			$return .= '<div class="vl-row"><span class="vl-label">'.$langs->trans('VerleihItemCategory').'</span><span>'.dol_escape_htmltag($this->itemcategory).'</span></div>';
		}
		if (!empty($this->manufacturer)) {
			$return .= '<div class="vl-row"><span class="vl-label">'.$langs->trans('VerleihManufacturer').'</span><span>'.dol_escape_htmltag($this->manufacturer).'</span></div>';
		}
		if (isset($arraydata['itemcount'])) {
			$return .= '<div class="vl-row"><span class="vl-label">'.$langs->trans('VerleihItems').'</span><span>'.((int) $arraydata['itemcount']).'</span></div>';
		}
		$return .= '</div>';
