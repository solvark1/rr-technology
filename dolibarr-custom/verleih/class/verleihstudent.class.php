...
/**
 * \file        htdocs/custom/verleih/class/verleihstudent.class.php
 * \ingroup     verleih
 * \brief       CRUD class for VerleihStudent (Schüler)
 */
...
/**
 * Class for VerleihStudent
 */
class VerleihStudent extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'verleihstudent';

	/**
	 * @var string Name of table without prefix where object is stored.
	 */
	public $table_element = 'verleih_student';
	...
	public $fields = array(
		...
		'fk_schoolclass' => array('type' => 'integer:VerleihSchoolClass:verleih/class/verleihschoolclass.class.php:1', 'label' => 'VerleihSchoolClass', 'picto' => 'fa-users', 'enabled' => 1, 'position' => 10, 'notnull' => -1, 'visible' => 1, 'index' => 1, 'css' => 'maxwidth300'),
		'lastname' => array('type' => 'varchar(64)', 'label' => 'Lastname', 'enabled' => 1, 'position' => 20, 'notnull' => 1, 'visible' => 1, 'searchall' => 1, 'showoncombobox' => 1, 'css' => 'minwidth150'),
		'firstname' => array('type' => 'varchar(64)', 'label' => 'Firstname', 'enabled' => 1, 'position' => 30, 'notnull' => 1, 'visible' => 1, 'searchall' => 1, 'showoncombobox' => 2, 'css' => 'minwidth150'),
		'studentnumber' => array('type' => 'varchar(32)', 'label' => 'VerleihStudentNumber', 'enabled' => 1, 'position' => 40, 'notnull' => 0, 'visible' => 1, 'searchall' => 1, 'css' => 'minwidth100'),
		'note' => array('type' => 'varchar(255)', 'label' => 'Note', 'enabled' => 1, 'position' => 50, 'notnull' => 0, 'visible' => 3, 'css' => 'minwidth300'),
	...
	{
		$url = dol_buildpath('/verleih/student_card.php', 1).'?id='.$this->id;
		$result = '<a href="'.$url.'">';
		if ($withpicto) {
	 ...
	 *        checkbox state), 'classlabel' (string, avoids a per-card query when supplied)
	 * @return string HTML
	 */
	public function getKanbanView($option = '', $arraydata = null)
	{
		...
		if ($classlabel === null && !empty($this->fk_schoolclass)) {
			...
			$classlabel = ($classobj->fetch($this->fk_schoolclass) > 0) ? $classobj->label.' ('.$classobj->schoolyear.')' : '';
		}
		$return .= '<div class="vl-sub">'.dol_escape_htmltag($classlabel ?? '').'</div>';
		if (!empty($this->studentnumber)) {
			$return .= '<div class="vl-body"><div class="vl-row"><span class="vl-label">'.$langs->trans('VerleihStudentNumber').'</span><span>'.dol_escape_htmltag($this->studentnumber).'</span></div></div>';
		}
