...
/**
 * \file        htdocs/custom/verleih/item_list.php
 * \ingroup     verleih
 * \brief       List page for VerleihItem (classic table + Kachel/kanban mode)
 *
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'verleihitemlist';
...
if (empty($reshook)) {
	...
	$objectclass = 'VerleihItem';
	$objectlabel = 'VerleihItem';
 ...
 */
$form = new Form($db);
$title = $langs->trans("VerleihItems");
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/verleih/item_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF'].($param ? '?'.$param : '')), '', $permissiontoadd);
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		foreach ($object->fields as $key => $val) {
			...
			} elseif ($key == 'itemcondition') {
				print $object->getLibCondition();
			} else {
