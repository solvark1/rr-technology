...
/**
 * \file        htdocs/custom/verleih/itemtype_list.php
 * \ingroup     verleih
 * \brief       List page for VerleihItemType (classic table + Kachel/kanban mode)
 *
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'verleihitemtypelist';
...
if (empty($reshook)) {
	...
	$objectclass = 'VerleihItemType';
	$objectlabel = 'VerleihItemType';
 ...
 */
$form = new Form($db);
$title = $langs->trans("VerleihItemTypes");
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/verleih/itemtype_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF'].($param ? '?'.$param : '')), '', $permissiontoadd);
