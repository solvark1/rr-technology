...
function rrRentingMigrate($db)
{
    ...
    $tables = array(
        ...
        "rr_renting_line" => "rowid INT AUTO_INCREMENT PRIMARY KEY, fk_booking INT NOT NULL, fk_asset INT NOT NULL, condition_out VARCHAR(20), condition_in VARCHAR(20), date_out DATETIME, date_return DATETIME, KEY idx_rr_booking_asset (fk_booking,fk_asset), KEY idx_rr_asset (fk_asset)",
    ...
    // Preserve one history row per delivery, including re-delivery of the same serial.
    $index=$db->query("SHOW INDEX FROM ".$p."rr_renting_line WHERE Key_name='uk_rr_booking_asset'");
    if(!$index) throw new RuntimeException($db->lasterror());
    if($db->num_rows($index) && !$db->query("ALTER TABLE ".$p."rr_renting_line DROP INDEX uk_rr_booking_asset, ADD INDEX idx_rr_booking_asset (fk_booking,fk_asset)")) throw new RuntimeException($db->lasterror());
}
