...
/**
 * \file        htdocs/custom/verleih/schoolclass_list.php
 * \ingroup     verleih
 * \brief       List page for VerleihSchoolClass (classic table + Kachel/kanban mode)
 *
 * Follows the structure of htdocs/modulebuilder/template/myobject_list.php (column
 * selection, mass actions, typed search fields, getKanbanView()) - see css_changes.md
 * section "Alles angleichen" for the rationale.
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'verleihschoolclasslist';
...
if (empty($reshook)) {
	...
	$objectclass = 'VerleihSchoolClass';
	$objectlabel = 'VerleihSchoolClass';
 ...
 */
$form = new Form($db);
$title = $langs->trans("VerleihSchoolClasses");
...
foreach ($search as $key => $val) {
	if (array_key_exists($key, $object->fields) && $search[$key] !== '') {
		...
		if (!empty($field_spec['arrayofkeyval'])) {
			$mode_search = 2;
		}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/verleih/schoolclass_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF'].($param ? '?'.$param : '')), '', $permissiontoadd);
