...
/**
 * \file        htdocs/custom/verleih/student_list.php
 * \ingroup     verleih
 * \brief       List page for VerleihStudent (classic table + Kachel/kanban mode)
 *
 * Follows the structure of htdocs/modulebuilder/template/myobject_list.php - see
 * css_changes.md section "Alles angleichen" for the rationale.
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'verleihstudentlist';
...
if (empty($reshook)) {
	...
	$objectclass = 'VerleihStudent';
	$objectlabel = 'VerleihStudent';
 ...
 */
$form = new Form($db);
$title = $langs->trans("VerleihStudents");
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/verleih/student_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF'].($param ? '?'.$param : '')), '', $permissiontoadd);
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		foreach ($object->fields as $key => $val) {
			...
			} elseif ($key == 'lastname' || $key == 'firstname') {
				// Clicking the name goes straight to this student's loan history,
				// not the edit card - see report_student.php.
				print '<a href="'.dol_buildpath('/verleih/report_student.php', 1).'?fk_student='.$object->id.'">';
